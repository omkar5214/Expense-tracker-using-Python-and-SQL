...
def start_app(user_id, name):
    ...
    def show_expenses():
        try:
            # Only show the logged-in user's expenses
            ...
            def update_selected():
                ...
                amount_entry = tk.Entry(update_window,width=30)
                amount_entry.pack(pady=5)

                amount_entry.insert(0,values[1].replace("₹", ""))
    ...
    def show_category_summary():
        try:
            results = get_category_totals(user_id)
            if not results:
                messagebox.showinfo(
                    "Category Summary",
                    "No expenses found."
